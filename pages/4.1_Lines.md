...
Given a point $\mathbf{p} = (1, 0, 2, 1, 4)$ and a direction vector $\mathbf{d} = (1, -1, 0, -1, 1)$ in $\mathbb{R}^5,$ find the equation of the line $\ell$ which goes through $\mathbf{p}$ in the direction of $\mathbf{d}$.
...
:::{dropdown} Solution (click to reveal)s
To calculate the direction vector $\mathbf{d}$ we need two points on the line. Choosing $x=0$ and $x=1$ then we have the two points $\mathbf{p}_1 = (0, 0, 1)$ and $\mathbf{p}_2 = (1, 0, 2)$ on $\ell_1$. Therefore
...
&emsp; (i) &emsp; Determine whether the two lines from {prf:ref}`parallel-lines-example` are perpendicular.
...
&emsp; (ii) &emsp; Find the equation of a line $\ell_2$ which is perpendicular to $(t,-t,1+t)$ at the point $\mathbf{p} = (-1,1,0).$
