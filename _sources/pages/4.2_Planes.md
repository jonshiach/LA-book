...
Recall that {prf:ref}`the dot product<dot-product-definition>` between two perpendicular vectors is zero. So if a plane is defined by the normal vector $\mathbf{n} = (n_x, n_y, n_z)^\mathrm{T}$ and a point with position vector $\mathbf{p}=(p_x, p_y, p_z)^\mathrm{T}$ which lies on the plane and if $(x,y,z)^\mathrm{T}$ is the position vector of another point the plane then
...
The point-normal form of the plane in $\mathbb{R}^3$ which passes through the point with position vector $\mathbf{p} = (p_x, p_y, p_z)^\mathrm{T}$ with normal vector $\mathbf{n} = (n_x, n_y, n_z)^\mathrm{T}$ is
...
A plane in $\mathbb{R}^3$ passes through the three points with position vectors $\mathbf{p}_1 = (2, 3, 1)^\mathrm{T}$, $\mathbf{p}_2 = (1, 0, -1)^\mathrm{T}$ and $\mathbf{p}_3 = (2, 1, 3)^\mathrm{T}$.
...
(ii) &emsp; Do the points with position vectors $\mathbf{p}_4 = (1, 2, 1)^\mathrm{T}$ and $\mathbf{p}_5 = (2, 4, 0)^\mathrm{T}$ lie on the plane?
