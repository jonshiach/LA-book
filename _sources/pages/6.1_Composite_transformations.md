...
Two linear transformations is defined as $T:(x, y, z)^\mathsf{T} \mapsto (2 x + 4 y, -x + 3 y, x + 2 y)^\mathsf{T}$ and $S:(x, y, z)^\mathsf{T} \mapsto (2x + y - z, 3x + z, y - 2z)^\mathsf{T}$, determine the composite linear transformation $S \circ T(\mathbf{u})$ for $\mathbf{u} = (x, y, z)^\mathsf{T}$.
