...
In $\mathbb{R}^n$ the shortest distance, $d$, between two points with position vectors $\mathbf{p}=(p_1, p_2, \ldots, p_n)^\mathrm{T}$ and $\mathbf{q} = (q_1, q_2, \ldots, q_n)^\mathrm{T}$ is the length of a straight line segment connecting them. If we think about this segment as of a vector, then
...
Find the shortest distance between the point with position vector $\mathbf{q} = (2,2,2)^\mathrm{T}$ and the line $(t,t-2, t+1)^\mathrm{T}$.
...
Find the shortest distance between the two skew lines $\ell_1$ and $\ell_2$ defined by $(t_1, 1 + 4t_1, 3 + 2 t_1)^\mathrm{T}$ and $(1, 1 + 2t_2, 3 + 4t_2)^\mathrm{T}$ respectively.
