...
Whilst {prf:ref}`matrix multiplication<matrix-multiplication-definition>` is defined for matrices of a certain size this is not true for the division of two matrices. However for certain square matrices it may be possible to compute the **inverse matrix** which can be multiplied by the matrix to achieve the same effect. If we consider the simpler case involving numbers, the multiplicative inverse of a number $x$ is a number denoted by $x^{-1}$ that when multiplied by $x$ results in the multiplicative identity 1, so the multiplicative inverse of $x$ is $1/x$. For matrices, the multiplicate identity is the identity matrix so we can define a multiplicative inverse with respect to matrix multiplication.
