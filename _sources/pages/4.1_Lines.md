...
Given a point with position vector $\mathbf{p} = (1, 0, 2, 1, 4)^\mathrm{T}$ and a direction vector $\mathbf{d} = (1, -1, 0, -1, 1)^\mathrm{T}$ in $\mathbb{R}^5$, find the equation of the line $\ell$ which passes through $\mathbf{p}$ in the direction of $\mathbf{d}$.
...
Three lines $\ell_1$, $\ell_2$ and $\ell_3$ in $\mathbb{R}^3$ are defined by the vector equations $(1 + t_1, -3 + 2t_1, t_1)^\mathrm{T}$, $(6 - t_2, -5 - 2t_2, t_2 - 1)^\mathrm{T}$ and $(6 + 2t_3, -11 + 2t_3, -2 - t_3)^\mathrm{T}$ respectively. Determine the points of intersection between the three lines (if possible).
...
:::{dropdown} Solution (click to reveal)
To calculate the direction vector $\mathbf{d}$ we need two points on the line. Choosing $x=0$ and $x=1$ then we have the two points with position vectors $\mathbf{p}_1 = (0, 0, 1)^\mathrm{T}$ and $\mathbf{p}_2 = (1, 0, 2)^\mathrm{T}$ on $\ell_1$. Therefore
...
Doing similar for $\ell_2$ we have $\mathbf{p}_1 = (0, 3, -1)^\mathrm{T}$, $\mathbf{p}_2 = (1, 3, -2)^\mathrm{T}$ so
...
(ii) &emsp; Find the equation of a line $\ell_2$ which is perpendicular to $(t,-t,1+t)^\mathrm{T}$ at the point with position vector $\mathbf{p} = (-1,1,0)^\mathrm{T}$.
