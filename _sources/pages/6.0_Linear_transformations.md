...
For example, let $V = \mathbb{R}^2$ and $W = \mathbb{R}^3$ then $T : V \to W$ defined by $T : (x, y)^\mathrm{T} \mapsto (x, y, 0)^\mathrm{T}$ is a linear transformation. Let $\mathbf{u} = (u_1, u_2)^\mathrm{T}, \mathbf{v} = (v_1, v_2)^\mathrm{T} \in \mathbb{R}^2$ and $\alpha \in F$ then
...
(i) &emsp; $T: \mathbb{R}^3 \to \mathbb{R}^2$ defined by $T: (x, y, z)^\mathrm{T} \mapsto (x, y)^\mathrm{T}$
...
So $T: (x, y, z)^\mathrm{T} \mapsto (x, y)^\mathrm{T}$ is a linear transformation.
:::

(ii) &emsp; $T: \mathbb{R}^3 \to \mathbb{R}^2$ defined by $T: (x, y, z)^\mathrm{T} \mapsto (x + 3, y)^\mathrm{T}$
...
Therefore $T(\mathbf{u} + \alpha \mathbf{v}) \neq T(\mathbf{u}) + \alpha T(\mathbf{v})$ so $T: (x, y, z)^\mathrm{T} \mapsto (x + 3, y)^\mathrm{T}$ is not a linear transformation. 
...
A linear transformation $T:\mathbb{R}^2 \to \mathbb{R}^2$ is defined by $T: (x, y)^\mathrm{T} \mapsto (3x + y, x + 2y)^\mathrm{T}$. Calculate the transformation matrix and use it to calculate $T\begin{pmatrix} 1 \\ 1 \end{pmatrix}$.
...
Applying the transformation matrix to $(1, 1)^\mathrm{T}$
...
The affect of applying a linear transformation $T: (x,y)^\mathrm{T} \mapsto (3x + y, x + 2y)^\mathrm{T}$ to the vector $(1,1)^\mathrm{T}$.
:::
...
Determine the inverse of the transformation $T: \mathbb{R}^2 \to \mathbb{R}^2$ defined by $T(x, y)^\mathrm{T} \mapsto (3 x + y, x + 2 y)^\mathrm{T}$ and calculate $T^{-1}\begin{pmatrix} 4 \\ 3 \end{pmatrix}$.
