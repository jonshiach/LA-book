...
Consider the diagram in {numref}`rotation-figure`. To determine the linear mapping $Rot(\theta): \mathbf{u} \mapsto \mathbf{v}$ we first consider the values of $\mathbf{u} = (u_1, u_2)^\mathbf{T}$ as the vector which points along the $x$-axis with the same magnitude as $\mathbf{u}$ rotated by an angle $\phi$ anti-clockwise about the origin. Forming a right-angled triangle with the angle $\phi$ and hypotenuse of length $|\mathbf{u}|$ then
...
Doing similar for $\mathbf{v} = (v_x, v_y)^\mathbf{T}$ we have
...
Rotate the vector $\mathbf{u} = (2, 1)^\mathbf{T}$ by angle $\theta = \frac{\pi}{2}$ anti-clockwise about the origin.
...
Applying the transformation to $\mathbf{u} = (2, 1)^\mathbf{T}$
...
The simplest reflection we can perform is to reflect a vector $\mathbf{u} =(u_1, u_2)^\mathbf{T}$ about the $x$-axis or $y$-axis. For example, to reflect about the $x$-axis we simply change the sign of the $y$ co-ordinate ({numref}`reflection-about-x-figure`), i.e.,
...
Reflect the vector $\mathbf{u} = \begin{pmatrix} 3 \\ -1 \end{pmatrix}$ about the line that passes through the origin and has gradient of 1. 
...
The scaling of a vector $\mathbf{u} = (u_1, u_2, \ldots, u_n)^\mathrm{T} \in \mathbb{R}^n$ by a scaling vector $\mathbf{s} = (s_1, s_2, \ldots, s_n)^\mathrm{T}$ is achieved by multiplying the corresponding elements in $\mathbf{u}$ and $\mathbf{s}$.
...
Scale the point with position vector $\mathbf{u} = \begin{pmatrix} 2 \\ 1 \end{pmatrix}$ by scaling scaling vector $\mathbf{s} = \begin{pmatrix} 2 \\ 3 \end{pmatrix}$.
