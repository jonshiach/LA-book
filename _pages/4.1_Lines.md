...
$$ \vec{r} = \vec{p} + t\vec{d}, $$(line-vector-equation)
...
In other words, every point on $\ell$ is obtained by adding some scalar multiple of $\vec{d}$ to $\vec{p}$ ({numref}`line-vector-equation-figure`). So $\ell$ is the line which passes through $\vec{p}$ in the direction of $\vec{d}$. We call the vector $\vec{d}$ a **direction vector** of the line.

```{figure} ../_images/4_vector_equation_of_a_line.svg
:name: line-vector-equation-figure
:width: 500

The position of any point on the line $\ell$ can be obtained by adding a scalar multiple of $\vec{d}$ to $\vec{p}$.
```

The equation of the line that passes through the two points with position vectors $\vec{p}_1$ and $\vec{p}_2$ can be determined by using {eq}`line-vector-equation` with $\vec{p} = \vec{p}_1$ and $\vec{d} = \vec{p}_2 - \vec{p}_1$. In particular, $\vec{d}$ and $\vec{p}$ are not unique and we can choose any point on the line for $\vec{p}$ and non-zero scalar multiple of $\vec{d}$ to express the same line. Take for instance the $x$-axis in $\mathbb{R}^3$. $\ell$ can be described as

$$ \vec{r} = \begin{pmatrix} 0 \\ 0 \\ 0 \end{pmatrix} + t \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix} = \begin{pmatrix} t \\ 0 \\ 0 \end{pmatrix}, $$
...
$$ \vec{r} = \begin{pmatrix} 71 \\ 0 \\ 0 \end{pmatrix} + t \begin{pmatrix} 5 \\ 0 \\ 0 \end{pmatrix} = \begin{pmatrix} 71 + 5t \\ 0 \\ 0 \end{pmatrix}, $$
