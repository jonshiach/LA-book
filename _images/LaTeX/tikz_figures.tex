...
% R3
% \begin{tikzpicture}
%     \tikzmath{\x=-30;\y=-\x;\z=90;\al=3;\lx=2;\ly=1;\lz=1;}
...
\begin{tikzpicture}
    \tikzmath{\x=0;\y=45;\z=90;\a=15;}
    \fill [gray!50,opacity=0.5] ($(0,0)+(\x:-2)+(\y:-2)$) -- ++(\x:6) -- ++(\y:4) -- ++(\x:-6) -- ++(\y:-4);
    \node [point,label=below:$\vec{p}$] (p) {};
    \node [point,label=below:$\vec{r}$] (r) at (\a:3) {};
