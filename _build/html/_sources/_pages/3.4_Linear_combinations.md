...
Express the vector $\vec{v} = (7, -2, -11)$ as a linear combination of the vectors
...
A special type of vector is a **basis vector** which all other vectors in the space can be represented as a linear combination of the basis vectors (we will cover basis in more detail [later](basis-section)). In a Cartesian space the simplest basis vectors are unit vectors that point in the co-ordinate directions. In $\mathbb{R}^3$ we use the basis vectors $\vec{i} = (1, 0, 0)$, $\vec{j} = (0, 1, 0)$ and $\vec{k} = (0, 0, 1)$ ({numref}`basis-vectors-figure`).
...
Using basis vectors we can represent any vector, $\vec{a} = (a_1, a_2, a_3)$ say, as a linear combination of $\vec{i}$, $\vec{j}$ and $\vec{k}$, i.e.,
