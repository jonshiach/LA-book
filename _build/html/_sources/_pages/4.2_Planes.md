...
Recall that {prf:ref}`the dot product<dot-product-definition>` between two perpendicular vectors is zero. So if a plane is defined by the normal vector $\vec{n} = (n_x, n_y, n_z)$ and a point with position vector $\vec{p}=(p_x, p_y, p_z)$ which lies on the plane and if $(x,y,z)$ is the position vector of another point the plane then
...
A plane in $\mathbb{R}^3$ passes through the three points with position vectors $\vec{p}_1 = (2, 3, 1)$, $\vec{p}_2 = (1, 0, -1)$ and $\vec{p}_3 = (2, 1, 3)$.
...
(ii) &emsp; Do the points with position vectors $\vec{p}_4 = (1, 2, 1)$ and $\vec{p}_5 = (2, 4, 0)$ lie on the plane?
