...
(i) &emsp; Show that  $\{ \vec{v}_1, \vec{v}_2 \}$ where $\vec{v}_1 = (2, 1)$ and $\vec{v}_2 = (4, 3)$ is a spanning set for $\mathbb{R}^2$.
...
Lets suggest $\left\{ \vec{i} = (1, 0, 0), \vec{j} = (0, 1, 0), \vec{k} = (0, 0, 1) \right\}$ as a spanning set for $\mathbb{R}^3$. We need to show that the following system has a non-trivial solution

$$ \begin{align*}
    \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix} \begin{pmatrix} \alpha_1 \\ \alpha_2 \end{pmatrix} = \begin{pmatrix} a \\ b \end{pmatrix},
\end{align*} $$

for $a, b \in \mathbb{R}$. The determinant of the coefficient matrix is $\det(A) = 1$ so this is non-singular and $\{\vec{i}, \vec{j}, \vec{k}\}$ is a spanning set for $\mathbb{R}^3$.
```
````
