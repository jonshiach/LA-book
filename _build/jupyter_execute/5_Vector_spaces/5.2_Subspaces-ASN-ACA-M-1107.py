...
# 
# \mathbf{u} + \alpha \mathbf{v} \in W,
# ```
# 
# where $\mathbf{u}, \mathbf{v} \in W$ and $\alpha \in F$.
# ````
# 
...
# 
# ```{dropdown} Solution
# The zero polynomial $0 \in W$ so $W$ is non-empty. Let $p(x), r(x) \in W$ and $\alpha \in \mathbb{R}$, we need to show that $p(x) + \alpha r(x) \in W$. By the definition of an even function we know that $p(-x) = p(x)$ and $r(-x) = r(x)$ so
# 
# \begin{align*}
#     p(-x) + \alpha r(-x) = p(x) + \alpha r(x),
# \end{align*}
# 
# so $p(-x) + \alpha r(-x) \in W$ therefore $W$ is a subspace.
# ```
# ````
# 
# ## Non-examples of Subspaces
# 
# Not all subsets of a vector space will be a subspace. For example, define $p(x)$ as the polynomial 
# 
# \begin{align*}
#     p(x)=a_0 + a_1 x + a_2 x^2 + \cdots + a_n x^n = \sum_{i = 0}^n a_ix^i ,
# \end{align*}
# 
# with $a_n\neq 0$, we say $p(x)$ has degree $n$ and write $\operatorname{deg}(p(x))=n$. Let 
# 
# \begin{align*}
#     W = \{p(x) \in P_n(\mathbb{R}) : \operatorname{deg}(p(x)) \in \{0,2,4\}\}
# \end{align*}
# 
# i.e, those polynomials of even degree at most five together with the zero polynomial. Is $W$ a subspace of $P(\mathbb{R}_5)$?
# 
# Let $p(x) = x^2$ and $r(x) = -x ^ 2 + x$ then $p(x), r(x) \in W$ but $p(x) + r(x) = x^2 - x^2 + x = x \notin W$ since $\operatorname{deg}(x) = 1$ is odd. Therefore $W$ is not a subspace of $P(\mathbb{R}_5)$. Here we came up with a single counter example to show that the linear combination $p(x) + r(x) \notin W$. 
# 
# This counterexample is great but how does one come up with such a counterexample if you can't immediately think of one? If you do not see an obvious counterexample, try and construct one: we are looking for two even-degree polynomials, whose sum is not an even polynomial. Let's start at the lowest degree: $0$. The sum of two polynomials of degree $0$ is just the sum of two real numbers, so there is no counterexample here. Let's try degree $2$. Now we need to find: $p(x) = ax^2 + bx + c$ and $r(x) = dx^2 + ex + f$, such that $p(x) + r(x)$ has an odd degree. Now 
# 
# \begin{align*}
#     p(x) + r(x) = (a + d)x^2 + (b + e)x + (c + f)
# \end{align*}
# 
# has degree one when $(a + d) = 0$ and $(b + e) \neq 0$. So let's take $a = 1$, $d = -1$ and $b = 0$, $e = 1$ (we don't care about $c$ or $f$, so let's just set both to be 0). This yields $p(x) = x^2$ and $r(x) = -x^2 + x$, which was our counterexample.
# 
