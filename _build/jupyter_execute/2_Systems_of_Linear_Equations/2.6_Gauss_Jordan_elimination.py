...
# 
# ```{important}
# When using Gauss-Jordan elimination with partial pivoting the row operations for reducing the elements in the pivot column to zero are performed prior to dividing the pivot row by the pivot.
# ```
# 
# ## Python code
# 
# The Python code below defines the function `rref()` that uses Gauss-Jordan elimination to calculate the reduced row echelon form of a matrix `A`.
...
# This has been used to calculate the solution to the system of linear equations from [example 2.6](gje-example).
...
# ````{note} 
# The SymPy library has a command for calculating the reduced row echelon form of a matrix which is `A.rref()`.
# ````
# 
