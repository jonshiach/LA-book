...
# ## Python code
# 
# The SymPy command for calculating the rank of a matrix `A` is `A.rank()`. The code below calculates the ranks of the coefficient matrix and the augmented matrix from [example 2.8](rank-example).
...
print(f"(i)   rank(A) = {A.rank()}, rank((A|b)) = {Ab.rank()}")
...
print(f"(ii)  rank(A) = {A.rank()}, rank((A|b)) = {Ab.rank()}")
...
print(f"(iii) rank(A) = {A.rank()}, rank((A|b)) = {Ab.rank()}")
