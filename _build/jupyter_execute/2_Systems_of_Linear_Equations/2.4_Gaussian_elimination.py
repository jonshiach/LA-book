...
# ## Python code
# 
# The Python code below defines the function `gelim()` that uses Gaussian elimination to determine the row echelon form of a matrix `A`.
...
# The following code defines the system of linear equations from part (ii) of [example 2.4](ge-example), forms the augmented matrix and uses the `gelim()` function to calculate the row echelon form of the augmented matrix.
...
# Calculate row echelon form of the augmented matrix
Ab = gelim(Ab)
display(Ab)
